...
class AbstractApi(object) :
    ...
    def httpCall(self, urlType, args=None) : 
        ...
        for retryCnt in range(0, 3) :
            if 'POST' == method :
                ...
                if args.has_key("uploadFile"):
                    uploadFile = args.get("uploadFile")
                    fileType = args.get("type")
                    response = self.__post_file(url, fileType, uploadFile)
                else:
    ...
    def __post_file(self, url, type, filepath):
        realUrl = self.__appendToken(url) + "&type=%s" % type
        media_file = {"media":("test.png", open(filepath,"rb"),"application/octet-stream")}
